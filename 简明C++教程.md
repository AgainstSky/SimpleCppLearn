...
```c++
vector<int> v(10,1);// v含有十个1
vector<int> v(10);//v含有十个int，这些int会被默认初始化为0
string s(10,'A');//s是一个10个A组成的字符串
std::vector<string> v{10,"sss"}; //这样子也是可行的，v是含有十个初始值为sss的string的容器
```

​	对于上面第二行的直接初始化代码，vector容器里存储的元素会被默认初始化，但如果是一些不支持默认初始化的类型，这行语句将报错，这种情况必须指定初始化值。



> 使用 = 拷贝初始化时只能提供一个值
>
> 如果是类内变量初始化只能使用 拷贝初始化 和 列表初始化，不能使用 （）初始化
>
> 如果提供的是一系列数据时，只能使用 列表初始化 ，不能使用 （）初始化
...
​	迭代器可以提供一种对容器元素的访问，不同于下标访问，迭代器支持所有的标准库容器，而下标访问只有有限的标准库容器提供支持。string虽然不是容器，但是很多操作和容器类似，它也支持迭代器操作。

​	类似于指针，迭代器也提供了对对象的间接访问，其对象是容器中元素或者string中的字符，迭代器能访问某个元素，也能从一个元素移动到另一个元素

```c++
std::vector<int> v(10,1);
auto begin = v.begin(),end = v.end();//通过begin和end函数获取迭代器，支持迭代器的类都会提供这两个函数，其中begin指向第一个元素，end指向最后一个元素的下一个元素，所以也被称为尾后迭代器（可以这么理解 v[9]是v中最后一个元素，而end指向的时 v[10]，这是一个不存在的元素，所以不能通过end直接操作它指向的元素）
```

> 如果容器为空，则begin和end都指向同一个位置，这两个迭代器进行 == 比较为true

​	下面是一些迭代器支持的运算符操作

```c++
*iter;//返回iter指向的元素的引用
iter->m;//访问iter指向元素的m成员
++iter;//使迭代器指向下一个元素
--iter;//使迭代器指向上一元素
iter1 == iter2;//比较两个迭代器比较的时他们是否指向同一个元素
iter1 != iter2;
```
